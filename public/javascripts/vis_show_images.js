...
function presentImg(imgData, showAnnotation, sortedKey = 0, imgSize = 1, currentPage) {
    ...
    var imtype_info = document.getElementById("image-type-info");
    var color_usage = document.getElementById("color-usage");
    var color_legend = document.getElementById("color-legend");
    var color_mapping = document.getElementById("color-map");
    var color_number = document.getElementById("color-number-info");
    ...
    $('.image-a').click(function(e) {
        ...
        let email_text = "mailto:biylc2@nottingham.ac.uk?subject=" +
            "VisImageNavigator: Update " + oriImageName + "&body=Dear Lin,%0D%0A%0D%0AMay I request to update the image file " +
            oriImageName + " please?%0D%0A%0D%0A best regards,%0D%0A"
        $("#email-btn").attr("href", email_text);
        //console.log(imgData[id]['paper_keyauthor'].replace(/,/g, '; ') == "");
        // if (checkEmptyString(imgData[id]['paper_keyauthor'])) {
        //     $("#keyword-info").css("color", "#99a6ad");
        //     keyword_info.innerHTML = "none supplied";
        // } else {
        //     $("#keyword-info").css("color", "#eeeeee");
        //     keyword_info.innerHTML = imgData[id]['paper_keyauthor'].replace(/,/g, '; ');
        // }
    ...
    var updateModalImage = function() {
        ...
        paper_info.innerHTML = imgData[gIndex]['paper_title'];
        author_info.innerHTML = imgData[gIndex]['paper_author'].replace(/;/g, '; ');
        ...
        if(imgData[gIndex]['color_number'] < 0)
            ...
            color_number.innerHTML = imgData[gIndex]['color_number'];
        ...
        $("#email-btn").attr("href", email_text);
        //console.log(imgData[id]['paper_keyauthor'].replace(/,/g, '; ') == "");
        // if (checkEmptyString(imgData[gIndex]['paper_keyauthor'])) {
        //     $("#keyword-info").css("color", "#99a6ad");
        //     keyword_info.innerHTML = "none supplied";
        // } else {
        //     $("#keyword-info").css("color", "#eeeeee");
        //     keyword_info.innerHTML = imgData[gIndex]['paper_keyauthor'].replace(/,/g, '; ');
        // }
...
function presentPaperCards(paperData, totalCount) {
    ...
    `;
        ...
        paperData[year].forEach((paper, i) => {
            ...
            var doi = paper['doi'].replace('/', '-');
            let title = paper['paper_title'];
            let author = paper['paper_author'].replace(/;/g, '; ');
            let year = paper['year'];
...
function presentUPPapers(paperData, totalCount) {
    ...
    for (let paperIndex = 0; paperIndex < paperData.length; paperIndex++) {
        let paperTitle = paperData[paperIndex]['paper_title'];
        //let keywords = paperData[paperIndex]['paper_keyauthor'];
        let paperUrl = paperData[paperIndex]['paper_url'];
        let author = paperData[paperIndex]['paper_author'].replace(/;/g, '; ');
        let conf = paperData[paperIndex]['conference'];
        let doi = paperData[paperIndex]['doi'];
        let year = paperData[paperIndex]['year'];
        let firstPage = paperData[paperIndex]['paper_first'];

        if (firstPage == 6666) {
            firstPage = "";
        }
        let lastPage = paperData[paperIndex]['paper_last'];
        let paper_div_id = 'p-' + paperIndex;
        var paper_div = document.createElement("div");
        paper_div.className = "paper-div";
        
        // let keywordsClass = 'keywords-1';
        // console.log(keywords+" paper title"+ paperTitle);
        ...
        */
        ...
        <div class='paper-panel row' id=${paper_div_id}>
            ...
            <span class='paperAuthors' style="text-align:left;">${author}, ${firstPage}-${lastPage}, ${conf}, ${year}</span>
        </div>
        `;
    ...
    $('.vis-img').click(function(e) {
        ...
        paper_info.innerHTML = imgDataDic[id]['paper_title'];
        author_info.innerHTML = imgDataDic[id]['paper_author'].replace(/;/g, '; ');
        ...
        color_number.innerHTML = imgDataDic[id]['color_number'];
        ...
        let email_text = "mailto:biylc2@nottingham.ac.uk?subject=" +
            ...
            oriImageName + " please?%0D%0A%0D%0A best regards,%0D%0A"
        $("#email-btn").attr("href", email_text);
        // if (checkEmptyString(imgDataDic[id]['paper_keyauthor'])) {
        //     $("#keyword-info").css("color", "#99a6ad");
        //     keyword_info.innerHTML = "none supplied";
        // } else {
        //     $("#keyword-info").css("color", "#eeeeee");
        //     keyword_info.innerHTML = imgDataDic[id]['paper_keyauthor'].replace(/,/g, '; ');
        // }
    ...
    var updateModalImage = function() {
        ...
        paper_info.innerHTML = paperImgData[gIndex]['paper_title'];
        author_info.innerHTML = paperImgData[gIndex]['paper_author'].replace(/;/g, '; ');
        ...
        $("#email-btn").attr("href", email_text);
        //console.log(imgData[id]['paper_keyauthor'].replace(/,/g, '; ') == "");
        // if (checkEmptyString(paperImgData[gIndex]['paper_keyauthor'])) {
        //     $("#keyword-info").css("color", "#99a6ad");
        //     keyword_info.innerHTML = "none supplied";
        // } else {
        //     $("#keyword-info").css("color", "#eeeeee");
        //     keyword_info.innerHTML = paperImgData[gIndex]['paper_keyauthor'].replace(/,/g, '; ');
        // }
...
function renderYearStatistics(divID, divWidth, divHeight) {
    ...
    var y = d3.scaleLinear()
        .domain([0, Math.max.apply(Math, scentDataArr.map(function(o) {
            return o.val;
        }))])
        .range([height, 0]);
    // svg.append("g")
